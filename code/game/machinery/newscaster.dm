	...
	if(ishuman(user) || issilicon(user)) // need abit of rewriting this to make it work for observers.
		...
		switch(screen)
			if(0)
				...
				dat += "<BR><FONT SIZE=1>Property of Syndicate</FONT>"
				...
				if(src.securityCaster)
					...
					dat+="<BR><A href='?src=\ref[src];menu_censor_channel=1'>Mark Feed Channel with Syndicate Notice</A>"
			...
			if(9)
				...
				if(src.viewing_channel.censored)
					dat+="<FONT COLOR='red'><B>ATTENTION: </B></FONT>This channel has been deemed as threatening to the welfare of the station, and marked with a Syndicate Notice.<BR>"
					dat+="No further feed story additions are allowed while the Notice is in effect.</FONT><BR><BR>"
				else
			...
			if(10)
				dat+="<B>Syndicate Feed Censorship Tool</B><BR>"
				...
				dat+="<BR><A href='?src=\ref[src];setScreen=[0]'>Cancel</A>"
			if(11)
				dat+="<B>Syndicate Notice Handler</B><HR>"
				dat+="<FONT SIZE=1>A Notice is to be bestowed upon the channel if the handling Authority deems it as harmful for the station's"
				dat+="morale, integrity or disciplinary behaviour. A Notice will render a channel unable to be updated by anyone, without deleting any feed"
				dat+="stories it might contain at the time. You can lift a Notice if you have the required access at any time.</FONT><HR>"
				if(isemptylist(news_network.network_channels))
					dat+="<I>No feed channels found active...</I><BR>"
				else
					for(var/datum/feed_channel/CHANNEL in news_network.network_channels)
			...
			if(13)
				dat+="<B>[src.viewing_channel.channel_name]: </B><FONT SIZE=1>\[ created by: <FONT COLOR='maroon'>[src.viewing_channel.author]</FONT> \]</FONT><BR>"
				dat+="Channel messages listed below. If you deem them dangerous to the station, you can <A href='?src=\ref[src];toggle_d_notice=\ref[src.viewing_channel]'>Bestow a Notice upon the channel</A>.<HR>"
				if(src.viewing_channel.censored)
					dat+="<FONT COLOR='red'><B>ATTENTION: </B></FONT>This channel has been deemed as threatening to the welfare of the station, and marked with a Syndicate Notice.<BR>"
					dat+="No further feed story additions are allowed while the Notice is in effect.</FONT><BR><BR>"
				else
...
/obj/machinery/newscaster/Topic(href, href_list)
	...
	if(href_list["set_channel_name"])
			...
			if(choice == "Confirm")
				if(input_param == 1)          //If input_param == 1 we're submitting a new wanted issue. At 2 we're just editing an existing one. See the else below
					...
					if(news_network.wanted_issue.is_admin_message)
						to_chat(usr, "The wanted issue has been distributed by a Syndicate higherup. You cannot edit it.")
						return FALSE
		...
		if(news_network.wanted_issue.is_admin_message)
			to_chat(usr, "The wanted issue has been distributed by a Syndicate higherup. You cannot take it down.")
			return FALSE
		...
		if(FC.is_admin_channel)
			to_chat(usr, "This channel was created by a Syndicate Officer. You cannot censor it.")
			return FALSE
		...
		if(MSG.is_admin_message)
			to_chat(usr, "This message was created by a Syndicate Officer. You cannot censor its author.")
			return FALSE
		...
		if(FC.is_admin_channel)
			to_chat(usr, "This channel was created by a Syndicate Officer. You cannot place a Notice upon it.")
			return FALSE
...
/obj/item/weapon/newspaper
	name = "newspaper"
	desc = "An issue of The Griffon, the newspaper circulating aboard Syndicate Space Stations."
	...
	if(ishuman(user))
		...
		switch(screen)
			if(0) //Cover
				...
				dat+="<DIV ALIGN='center'><FONT SIZE=2>Syndicate-standard newspaper, for use on Syndicate© Space Facilities</FONT></div><HR>"
			...
			if(1) // X channel pages inbetween.
				...
				if(C.censored)
					dat+="This channel was deemed dangerous to the general welfare of the station and therefore marked with a <B><FONT COLOR='red'>Notice</B></FONT>. Its contents were not transferred to the newspaper at the time of printing."
				else
