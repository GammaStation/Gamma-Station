...
/datum/game_mode/blob/send_intercept(var/report = 1)
	...
	switch(report)
		...
		if(1)
			...
			intercepttext += "<FONT size = 3><B>Syndicate Update</B>: Biohazard Alert.</FONT><HR>"
			intercepttext += "Reports indicate the probable transfer of a biohazardous agent onto SFS Gamma during the last crew deployment cycle.<BR>"
			intercepttext += "Preliminary analysis of the organism classifies it as a level 5 biohazard. Its origin is unknown.<BR>"
			intercepttext += "Syndicate has issued a directive 7-10 for SFS Gamma. The station is to be considered quarantined.<BR>"
			intercepttext += "Orders for all SFS Gamma personnel follows:<BR>"
		...
		if(2)
			...
			intercepttext += "<FONT size = 3><B>Syndicate Update</B>: Biohazard Alert.</FONT><HR>"
			intercepttext += "Directive 7-12 has been issued for SFS Gamma.<BR>"
