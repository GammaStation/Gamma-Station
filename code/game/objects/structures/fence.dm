...
/obj/structure/fence/examine(mob/user)
	...
	switch(hole_size)
		if(SMALL_HOLE)
			to_chat(user, "<span class='notice'>There is a small hole in \the [src].</span>")
		if(MEDIUM_HOLE)
			to_chat(user, "<span class='notice'>There is a large hole in \the [src].</span>")
		if(LARGE_HOLE)
			to_chat(user, "<span class='notice'>The [src] has been completely cut through.</span>")
			...
			if(current_stage == hole_size)
				...
				switch(++hole_size)
					if(SMALL_HOLE)
						...
						to_chat(user, "<span class='info'>This hole seems to be [FAT in user.mutations ? "way " : ""]too small to climb though, but you probably could throw something through it.</span>")
					if(MEDIUM_HOLE)
...
/obj/structure/fence/attack_hand(mob/user)
	if(user.a_intent == I_HURT)
		...
		playsound(src, 'sound/effects/fence_smash.ogg', 30, 1)
		shock(user, 100)
		return TRUE
	...
	if(hole_size == MEDIUM_HOLE)
		...
		if(do_after(user, CLIMB_TIME, src) && !shock(user, 70)) //70% chance to get shocked
...
/obj/structure/fence/Cross(atom/movable/mover, turf/target, height = 1.5, air_group = 0)
	...
	if(istype(mover) && mover.checkpass(PASSGRILLE))
		...
		if(istype(mover, /obj/item/projectile))
			return prob(30) //Fairly hit chance
		else
			return !density
...
/obj/structure/fence/door/secure/can_open(mob/user)
	...
	if((TK in usr.mutations) || (get_dir(src, user) == permitted_direction))
		return TRUE
	else
