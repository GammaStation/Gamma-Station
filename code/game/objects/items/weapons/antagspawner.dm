...
/obj/item/weapon/antag_spawner/borg_tele
	name = "Nanotrasen Cyborg Teleporter"
	desc = "A single-use teleporter used to deploy a Nanotrasen Cyborg on the field."
...
/obj/item/weapon/antag_spawner/borg_tele/proc/request_player(client/C)
	if(!C)
		return
	var/response = alert(C, "Nanotrasen requesting a personality for a NT borg. Would you like to play as one?", "Nanotrasen borg request", "Yes", "No")
	if(!C)
...
/obj/item/weapon/antag_spawner/borg_tele/proc/stop_search()
	if(requested_candidates.len > 0)
		...
		used = FALSE
		visible_message("\blue Unable to connect to Nanotrasen Central Command. Please wait and try again later or use the teleporter on your uplink to get your points refunded.")
