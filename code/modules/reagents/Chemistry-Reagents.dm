...
/datum/reagent/consumable/sprinkles/on_mob_life(mob/living/M)
	if(!..())
		return
	M.nutrition += nutriment_factor
	if(istype(M, /mob/living/carbon/human) && M.job in list("Police Officer", "Marshall", "Detective", "Sergeant", "Captain")) //if we want some FUN and FEATURES we should uncomment it
		M.heal_bodypart_damage(1, 1)
		M.nutrition += nutriment_factor
