...
/datum/random_map/New(var/seed, var/tx, var/ty, var/tz, var/tlx, var/tly, var/do_not_apply, var/do_not_announce, var/never_be_priority = 0)
	...
	if(!do_not_announce) message_admins("<span class='danger'>Generating [name].</span>")
	...
	for(var/i in 0 to max_attempts-1)
		if(generate())
			if(!do_not_announce) message_admins("<span class='danger'>[capitalize(name)] generation completed in [round(0.1*(world.timeofday-start_time),0.1)] seconds.</span>")
			return
	if(!do_not_announce) message_admins("<span class='danger'>[capitalize(name)] failed to generate ([round(0.1*(world.timeofday-start_time),0.1)] seconds): could not produce sane map.</span>")
