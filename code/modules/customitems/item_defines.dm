...
/obj/item/weapon/paper/fluff/sue_donem/atom_init()
	...
	info = "<B>Organic Carrier AIA and Standard Cyborgification Agreement</B><BR>\n<BR>\nUnder the authority of Syndicate Synthetic Intelligence Division, this document hereby authorizes an accredited Roboticist of the SFS Gamma or a deputized authority to perform a regulation lobotomisation upon the person of one '<I>Sue Donem</I>' (hereafter referred to as the Subject) with intent to enact a live Artificial Intelligence Assimilation (AIA) or live Cyborgification proceedure.<BR>\n<BR>\nNo further station authorization is required, and the Subject waives all rights as a human under Syndicate internal and external legal protocol. This document is subject to amendment under Syndicate internal protocol \[REDACTED\].<BR>\n<BR>\nSigned: <I>Sue Donem</I><BR>\n"

	stamp_text = (stamp_text=="" ? "<HR>" : "<BR>") + "<i>This paper has been stamped with the Syndicate Synthetic Intelligence Division rubber stamp.</i>"
