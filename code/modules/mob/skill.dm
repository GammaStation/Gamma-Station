...
/proc/setup_skills()
	if(SKILLS == null)
		SKILLS = list()
		for(var/T in (subtypesof(/datum/skill)-/datum/skill))
...
/mob/proc/GetSkillClass(points)
	// skill classes describe how your character compares in total points
	var/original_points = points
	if(ishuman(src))
		var/mob/living/carbon/human/H = src
		var/active_age = H.age
...
/proc/CalculateSkillPoints(var/list/skills)
	if(!skills || !skills.len) return 0
...
/proc/show_skill_window(var/mob/user, var/mob/M)
	...
	if(!M.mind)
		user << "There is no mind"
		return

	var/isadmin = 0
	if(user && user.client && user.client.holder && user.client.holder.rights)
		isadmin = 1
	var/list/skills = M.mind.skills
	if(!skills)
		skills = list()

	var/HTML = "<body>"
	HTML += "<b>Select your Skills</b><br>"
	var/points = CalculateSkillPoints(skills)
	HTML += "Current skill level: <b>[M.GetSkillClass(points)]</b> ([points])<br>"
	HTML += "<table>"
	var/mod = 0
	if(M.mind.special_role)
		mod = 1
	for(var/V in SKILLS)
		HTML += "<tr><th colspan = [isadmin?6:5]><b>[V]</b>"
		HTML += "</th></tr>"
		for(var/datum/skill/S in SKILLS[V])
			var/level = min(3, skills[S.ID] + mod)
			level = max(0, round(level, 1))
			...
			if(isadmin)
				HTML += "<th>[skills[S.ID]] <a href='byond://?src=\ref[user.client.holder];set_skill=\ref[M];skill=[S.ID]'>\[Set\]</a></th>"
			HTML += "</tr>"
	HTML += "</table>"
