/mob/living/carbon/ian/examine(mob/user)
	...
	if(!skipface && mouth && !(mouth.flags&ABSTRACT)) // Yesh, hide hand if face is obscured. Err, i mean mouth!
		if(mouth.blood_DNA)
			msg += "<span class='warning'>[t_He] [t_is] holding [bicon(mouth)] [mouth.gender==PLURAL?"some":"a"] [get_mess_desc(mouth.blood_color)]-stained [mouth.name] in [t_his] mouth!</span>\n"
		else if(mouth.wet)
