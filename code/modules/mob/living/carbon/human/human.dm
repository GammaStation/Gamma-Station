...
/mob/living/carbon/human/proc/is_damaged_organ(organ)
	...
	return FALSE // In case we didn't find anything.
		...
		switch(regenerating_organ_time)
			...
			if(20)
			if(30)
					visible_message("<span class='notice'>[src] stirs his [regenerating_bodypart.name]...</span>","<span class='userdanger'>You [species && species.flags[NO_PAIN] ? "notice" : "feel"] freedom in moving your [regenerating_bodypart.name]</span>")				else
			if(50)
			if(65)
		if(prob(50))
...
/mob/living/carbon/human/Topic(href, href_list)
	...
	if(ismob(usr))
		var/mob/M = usr
		if((TK in M.mutations) && in_range(M.get_active_hand(), src))
			range_check = TRUE
